...
class AudioConverterApp:
    def __init__(self, master):
        self.master = master
        master.title("Công cụ chuyển đổi hàng loạt Âm thanh/P3")
    ...
    def create_widgets(self):
        ...
        ttk.Radiobutton(mode_frame, text="Âm thanh sang P3", variable=self.mode,
                        value="audio_to_p3", command=self.toggle_settings,
                        width=15).grid(row=0, column=0, padx=5)
        ttk.Radiobutton(mode_frame, text="P3 sang Âm thanh", variable=self.mode,
    ...
    def convert_audio_to_p3(self, target_lufs, input_files):
        """Logic chuyển đổi Âm thanh sang P3"""
    ...
    def convert_p3_to_audio(self, input_files):
        """Logic chuyển đổi P3 sang Âm thanh"""
        for input_path in input_files:
            try:
                filename = os.path.basename(input_path)
                base_name = os.path.splitext(filename)[0]
